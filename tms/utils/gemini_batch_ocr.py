...
def gemini_extract_passengers_batch(
    ...
    """
    One request, multiple images/PDFs.
    Returns dict:
      {
        "passengers": [
            {"full_name": "...", "id_no": "...", "nationality": "...", "raw_text": "...", "confidence": 0.0-1.0}
        ],
        "raw_response_text": "..."
      }
    """
    client = genai.Client()  # uses GEMINI_API_KEY / GOOGLE_API_KEY env vars
...
Rules:
- passengers array length MUST equal number of input documents.
- Each passengers[i] corresponds to document i.
- Never invent values. If unknown, use "".
...
"""
    ...
    try:
        ...
        # return raw so caller can log it
        return {"passengers": [], "raw_response_text": text}

    passengers = data.get("passengers") if isinstance(data, dict) else None
    if not isinstance(passengers, list):
        passengers = []

    return {"passengers": passengers, "raw_response_text": text}
