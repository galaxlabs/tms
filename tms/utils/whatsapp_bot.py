# tms/utils/whatsapp_bot.py  (example)
# Simplified bot:
# - Ignore sender if not Staff
# - Collect passenger count + docs (image/pdf)
# - Auto-pick RANDOM route from fixed list (Jeddah/Makkah/Madinah)
# - Create/Reuse Trip for 1 hour
...
#     # create it (distance/duration will be filled using your function)
#     route = frappe.new_doc("Route")
#     route.from_city = from_city
#     route.to_city = to_city

#     try:
#         # your existing whitelisted method
#         from tms.transport_management_system.doctype.route.route import get_distance
#         info = get_distance(from_city, to_city)
#         route.from_city = info.get("from_city") or from_city
#         route.to_city = info.get("to_city") or to_city
...
#         # reuse only if active + created within 1 hour
#         created_ok = True
#         try:
#             created_ok = (now_datetime() - t.creation) <= timedelta(hours=1)
#         except Exception:
#             created_ok = True
...
#         # also reset collection counters for the new trip
#         _update_contact_state(
#             contact,
#             bot_state="WAITING_PASSENGER_COUNT",
#             expected_passengers=0,
